...
for sim=1:simulation
 ...
 cv=cvpartition(size(class1_data,1),'HoldOut',0.1);
index=cv.test;
class1_train=class1_data(index,:);
class1_test=class1_data(~index,:);

cv=cvpartition(size(class2_data,1),'HoldOut',0.1);
index=cv.test;
class2_train=class2_data(index,:);
class2_test=class2_data(~index,:);

cv=cvpartition(size(class3_data,1),'HoldOut',0.1);
index=cv.test;
...
%a=selected_data;
b=ones(size(a_train,1),size(a_train,2));
c=zeros(3*ndata,3);
